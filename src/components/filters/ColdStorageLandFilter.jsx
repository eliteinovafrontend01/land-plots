...
import { 
  ...
  Square, Home, Bus, CheckCircle, TrendingUp, Clock, Warehouse,
  Truck, Battery, Droplets, Wifi, HardDrive, ShieldCheck,
  Trees, Fence, Car, Sparkles, Sun, Award, Landmark, FileCheck,
  Box, Package, Thermometer, Snowflake, Anchor, Snowflake as Ice,
  Droplet as WaterDrop, Apple, Fish, Leaf, Pill, ShoppingBag
} from 'lucide-react';
...
// Checkbox Group Component - Two Column Grid
...
const ColdStorageLandFilter = ({ activeTab = 'Buy', onFilterChange, onClose, onTabChange }) => {
  ...
  const [filters, setFilters] = useState({
    ...
    listingType: [],
    // Location Details
    city: '', taluk: '', locality: '', industrialZone: '', landmark: '', pincode: '',
    mainRoadAccess: '', highwayConnectivity: '', nearbyTransport: '',
    // Price/Lease/Rent/Sell Details
    minPrice: '', maxPrice: '', priceNegotiable: '', loanRequired: '',
    minRent: '', maxRent: '', securityDeposit: '', rentNegotiable: '',
    minLeaseAmount: '', maxLeaseAmount: '', advanceAmount: '', leaseDuration: '', leaseNegotiable: '',
    minSellPrice: '', maxSellPrice: '', sellPriceNegotiable: '', maintenanceCharges: '', propertyTax: '',
    pricePerUnit: '', preferredPricePerUnit: '',
    registrationChargesIncluded: '',
    // Land Details
    landArea: '', landAreaUnit: 'acres', builtupArea: '', openYardArea: '', truckParkingCapacity: '',
    loadingUnloadingSpace: '', plotLength: '', plotWidth: '', facing: '', cornerPlot: '', 
    roadWidth: '', boundaryWall: '', levelledLand: '', waterLoggingFree: '',
    minRoadWidthRequired: '',
    // Cold Storage Suitability
    coldStorageSuitability: [],
    // Infrastructure & Utilities
    infrastructureUtilities: [],
    // Amenities
    amenities: [],
    // Approval & Legal Details
    dtcpApproved: '', cmdaApproved: '', coldStorageApproval: '', fssaiCompliance: '',
    pollutionClearance: '', environmentalClearance: '', fireNOC: '',
    encumbranceFree: '', pattaAvailable: '', loanEligible: '', landSurveyCompleted: '', titleDeedVerified: '',
    rentalAgreementAvailable: '', leaseAgreementAvailable: '',
    // Availability
    readyToOccupy: '', immediatePossession: '', vacantLandAvailable: '', operationalFacilityAvailable: '',
    readyToRegister: '',
    // Nearby Access
    nearbyAccess: [],
    // Contact
  ...
  const rentalDurationOptions = [
    ...
    { value: 'longterm', label: 'Long-Term Rent' }
  ];
  ...
  const coldStorageSuitabilityOptions = [
    'Agricultural Produce Storage', 'Dairy Product Storage', 'Frozen Food Storage', 'Seafood Storage',
    'Meat Processing Storage', 'Pharmaceutical Cold Storage', 'FMCG Cold Storage', 'Multi-Temperature Storage',
    'Ripening Chamber Facility', 'Export Storage Facility'
  ];

  const infrastructureUtilitiesOptions = [
    'Electricity Connection Available', '3-Phase Power Supply', 'High Tension (HT) Power Available',
    'DG Backup Available', 'Transformer Nearby', 'Borewell Facility', 'Water Connection Available',
    'Drainage Facility', 'Sewage Connection', 'Internet / Fiber Connectivity', 'Heavy Vehicle Access',
    'Container Truck Access', 'Loading Dock Available', 'Dock Levelers Available', 'Refrigeration System Available'
  ];

  const amenitiesOptions = [
    'Gated Cold Storage Park', 'Security / Watchman', 'CCTV Surveillance', 'Internal Roads',
    'Street Lights', 'Parking Space', 'Fire Safety System', 'Worker Accommodation',
    'Office Space Available', 'Common Utilities Available'
  ];

  const nearbyOptions = [
    'National Highway Access', 'Railway Freight Terminal Nearby', 'Airport Cargo Terminal Nearby',
    'Seaport Nearby', 'Agricultural Market Nearby', 'Food Processing Zone Nearby', 'Industrial Corridor Access'
  ];
  ...
  const mainSections = [
    ...
    { id: 'coldstorage', label: '❄️ Cold Storage', icon: <Snowflake className="w-3.5 h-3.5" /> },
  ...
  const applyFilters = () => {
    const filtersToSend = { ...filters, purpose: currentTab, propertyType: 'Cold Storage Land' };
    ...
    <div className="space-y-3">
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <div>
            ...
            <input type="text" value="Cold Storage Land" disabled className="w-full px-2 py-1.5 rounded border border-teal-300 bg-gray-50 text-xs text-gray-600" />
          </div>
          <div>
          ...
          <div className="md:col-span-2">
            ...
            <div className="flex gap-4">
              {['Owner', 'Agent', 'Developer', 'Builder'].map(type => (
      ...
      <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <input type="text" placeholder="Industrial Zone / Food Park / Logistics Park" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.industrialZone} onChange={(e) => handleInputChange('industrialZone', e.target.value)} />
          ...
          <input type="text" placeholder="Highway Connectivity" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.highwayConnectivity} onChange={(e) => handleInputChange('highwayConnectivity', e.target.value)} />
          <input type="text" placeholder="Nearby Transport Access" className="w-full px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.nearbyTransport} onChange={(e) => handleInputChange('nearbyTransport', e.target.value)} />
        </div>
      </div>
    </div>
  );
        ...
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              ...
              <input type="number" placeholder="Security Deposit / Advance Amount (₹)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.securityDeposit} onChange={(e) => handleInputChange('securityDeposit', e.target.value)} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <input type="number" placeholder="Rent Per Sq.ft / Acre (₹)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.pricePerUnit} onChange={(e) => handleInputChange('pricePerUnit', e.target.value)} />
              <CustomSelect label="Rental Agreement Duration" options={rentalDurationOptions} value={filters.leaseDuration} onChange={(val) => handleInputChange('leaseDuration', val)} placeholder="Select Duration" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              ...
              <YesNoRadioGroup label="Property Tax Included" name="propertyTax" value={filters.propertyTax} onChange={(val) => handleRadioChange('propertyTax', val)} />
            </div>
          </div>
        </div>
      );
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <div className="grid grid-cols-2 gap-2">
                <input type="number" placeholder="Min Budget (₹)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.minPrice} onChange={(e) => handleInputChange('minPrice', e.target.value)} />
                <input type="number" placeholder="Max Budget (₹)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.maxPrice} onChange={(e) => handleInputChange('maxPrice', e.target.value)} />
              </div>
              <input type="number" placeholder="Preferred Price Per Sq.ft / Acre (₹)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.preferredPricePerUnit} onChange={(e) => handleInputChange('preferredPricePerUnit', e.target.value)} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              ...
              <YesNoRadioGroup label="Registration Charges Included Preferred" name="registrationChargesIncluded" value={filters.registrationChargesIncluded} onChange={(val) => handleRadioChange('registrationChargesIncluded', val)} />
            </div>
          </div>
        </div>
      );
        ...
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              ...
              <YesNoRadioGroup label="Registration Charges Included" name="registrationChargesIncluded" value={filters.registrationChargesIncluded} onChange={(val) => handleRadioChange('registrationChargesIncluded', val)} />
            </div>
          </div>
        </div>
      );
        ...
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              ...
              <input type="number" placeholder="Advance Amount / Security Deposit (₹)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.advanceAmount} onChange={(e) => handleInputChange('advanceAmount', e.target.value)} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
  ...
  const renderLandSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          <input type="text" placeholder="Cold Storage Built-up Area (sq.ft)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.builtupArea} onChange={(e) => handleInputChange('builtupArea', e.target.value)} />
          <input type="text" placeholder="Open Yard Area (sq.ft)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.openYardArea} onChange={(e) => handleInputChange('openYardArea', e.target.value)} />
          <input type="text" placeholder="Truck Parking Capacity" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.truckParkingCapacity} onChange={(e) => handleInputChange('truckParkingCapacity', e.target.value)} />
          <input type="text" placeholder="Loading / Unloading Space (sq.ft)" className="px-2 py-1.5 rounded border border-teal-300 bg-white text-xs focus:outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500" value={filters.loadingUnloadingSpace} onChange={(e) => handleInputChange('loadingUnloadingSpace', e.target.value)} />
          ...
          <YesNoRadioGroup label="Levelled Land" name="levelledLand" value={filters.levelledLand} onChange={(val) => handleRadioChange('levelledLand', val)} />
          <YesNoRadioGroup label="Water Logging Free" name="waterLoggingFree" value={filters.waterLoggingFree} onChange={(val) => handleRadioChange('waterLoggingFree', val)} />
          {currentTab === 'Buy' && (
  ...
  const renderColdStorageSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      <h3 className="font-semibold text-teal-800 mb-2 flex items-center gap-1.5 text-sm"><Snowflake className="w-3.5 h-3.5" /> Cold Storage Suitability</h3>
      <div className="grid grid-cols-2 gap-1.5">
        {coldStorageSuitabilityOptions.map(option => (
          <label key={option} className="flex items-center gap-1.5 cursor-pointer group p-1 rounded hover:bg-teal-50 transition-colors">
            <input
              type="checkbox"
              checked={filters.coldStorageSuitability.includes(option)}
              onChange={() => handleArrayToggle('coldStorageSuitability', option)}
              className="w-3.5 h-3.5 rounded border border-teal-400 checked:bg-teal-500 checked:border-teal-500 accent-teal-500"
            />
            <span className="text-xs text-gray-700 group-hover:text-teal-500">{option}</span>
          </label>
  ...
  const renderLegalSection = () => (
    <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        ...
        <YesNoRadioGroup label="Industrial / Cold Storage Approval Available" name="coldStorageApproval" value={filters.coldStorageApproval} onChange={(val) => handleRadioChange('coldStorageApproval', val)} />
        <YesNoRadioGroup label="FSSAI Compliance Eligible" name="fssaiCompliance" value={filters.fssaiCompliance} onChange={(val) => handleRadioChange('fssaiCompliance', val)} />
        <YesNoRadioGroup label="Pollution Clearance Available" name="pollutionClearance" value={filters.pollutionClearance} onChange={(val) => handleRadioChange('pollutionClearance', val)} />
        ...
        <YesNoRadioGroup label="Fire NOC Available" name="fireNOC" value={filters.fireNOC} onChange={(val) => handleRadioChange('fireNOC', val)} />
    ...
    if (currentTab === 'Rent' || currentTab === 'Lease') {
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <YesNoRadioGroup label="Ready to Occupy" name="readyToOccupy" value={filters.readyToOccupy} onChange={(val) => handleRadioChange('readyToOccupy', val)} />
            ...
            <YesNoRadioGroup label="Vacant Cold Storage Land Available" name="vacantLandAvailable" value={filters.vacantLandAvailable} onChange={(val) => handleRadioChange('vacantLandAvailable', val)} />
            <YesNoRadioGroup label="Operational Cold Storage Facility Available" name="operationalFacilityAvailable" value={filters.operationalFacilityAvailable} onChange={(val) => handleRadioChange('operationalFacilityAvailable', val)} />
          </div>
        </div>
      );
    } else if (currentTab === 'Sell') {
      ...
      return (
        <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-lg p-3 border border-teal-200">
          ...
          <div className="space-y-2">
            <YesNoRadioGroup label="Ready to Register" name="readyToRegister" value={filters.readyToRegister} onChange={(val) => handleRadioChange('readyToRegister', val)} />
            <YesNoRadioGroup label="Immediate Possession" name="immediatePossession" value={filters.immediatePossession} onChange={(val) => handleRadioChange('immediatePossession', val)} />
            <YesNoRadioGroup label="Vacant Cold Storage Land Preferred" name="vacantLandAvailable" value={filters.vacantLandAvailable} onChange={(val) => handleRadioChange('vacantLandAvailable', val)} />
            <YesNoRadioGroup label="Operational Cold Storage Facility Preferred" name="operationalFacilityAvailable" value={filters.operationalFacilityAvailable} onChange={(val) => handleRadioChange('operationalFacilityAvailable', val)} />
          </div>
        </div>
      );
    }
      ...
      <div className="bg-white rounded-xl shadow-xl border border-teal-100 overflow-hidden flex flex-col" style={{ maxHeight: '85vh', width: '100%', maxWidth: '900px' }}>
          ...
          <div className="flex justify-between items-center px-3 py-2 bg-gradient-to-r from-teal-600 to-emerald-700">
            <div className="flex items-center gap-2">
              <div className="p-0.5 bg-white/20 rounded">
                <Snowflake className="w-4 h-4 text-white slow-rotate" />
              </div>
              <h3 className="text-white font-semibold text-sm">Filter Cold Storage Land</h3>
            </div>
            {onClose && (
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-3 space-y-3">
            ...
            {activeMainSection === 'coldstorage' && renderColdStorageSection()}
...
export default ColdStorageLandFilter;