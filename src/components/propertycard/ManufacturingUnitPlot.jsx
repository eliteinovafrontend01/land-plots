...
import { ManufacturingUnitPlotData } from "../data/ManufacturingUnitPlotData";

const PAGE_NAME = "ManufacturingUnit Plot";
...
const ManufacturingUnitPlot = () => {
  const [showLoginModal, setShowLoginModal] = useState(false);
  const [selectedProperty, setSelectedProperty] = useState(null);
  const [showContactInfo, setShowContactInfo] = useState(false);
  const [filteredProperties, setFilteredProperties] = useState(ManufacturingUnitPlotData);
          ...
          <div className="flex flex-col gap-5">
            {filteredProperties.length > 0 ? (
              ...
              <div className="w-full bg-white rounded-2xl shadow-2xl border border-teal-100 p-8 text-center">
                ...
                <p className="text-xs text-slate-500">No ManufacturingUnitPlot apartments available at the moment.</p>
              </div>
            )}
          </div>
        </div>
...
export default ManufacturingUnitPlot;