...
import { PetrolBunkPlotData } from "../data/PetrolBunkPlotData";

const PAGE_NAME = "PetrolBunkPlot";
...
const PetrolBunkPlot = () => {
  const [showLoginModal, setShowLoginModal] = useState(false);
  const [selectedProperty, setSelectedProperty] = useState(null);
  const [showContactInfo, setShowContactInfo] = useState(false);
  const [filteredProperties, setFilteredProperties] = useState(PetrolBunkPlotData);
          ...
          <div className="flex flex-col gap-5">
            {filteredProperties.length > 0 ? (
              ...
              <div className="w-full bg-white rounded-2xl shadow-2xl border border-teal-100 p-8 text-center">
                ...
                <p className="text-xs text-slate-500">No PetrolBunkPlot apartments available at the moment.</p>
              </div>
            )}
          </div>
        </div>
