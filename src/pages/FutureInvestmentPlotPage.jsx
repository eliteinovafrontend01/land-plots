...
import FutureInvestmentPlotFilter from "../components/filters/FutureInvestmentPlotFilter";
import FutureInvestmentPlot from "../components/propertycard/FutureInvestmentPlot";

const FutureInvestmentPlotPage = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const [activeButton, setActiveButton] = useState("Buy");
  const [activeLandType, setActiveLandType] = useState("FutureInvestment Plot");
  ...
  useEffect(() => {
    ...
    if (activeType) {
      ...
      setActiveLandType("FutureInvestment Plot");
    }
  }, [location.pathname]);
  ...
  const SearchBar = () => (
    <div className="relative flex-1 group">
      ...
      <input
        type="text"
        placeholder="Search FutureInvestment Plots by city, locality, or project name"
...
export default FutureInvestmentPlotPage;