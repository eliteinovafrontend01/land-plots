...
import DTCPPlotFilter from "../components/filters/DTCPPlotFilter";
import DTCPPlot from "../components/propertycard/DTCPPlot";

const DTCPPlotPage = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const [activeButton, setActiveButton] = useState("Buy");
  const [activeLandType, setActiveLandType] = useState("DTCP Plot");
  ...
  const SearchBar = () => (
    <div className="relative flex-1 group">
      ...
      <input
        type="text"
        placeholder="Search DTCP plots by city, locality, or project name"
        ...
        <section className="w-full relative flex items-center justify-center group py-2 md:py-4">
          ...
          <div className="max-w-none mx-auto px-4 sm:px-6 relative z-10 text-center w-full flex flex-col items-center justify-center gap-2">
            ...
            <p className="text-sm md:text-base lg:text-lg text-white/90 max-w-3xl mx-auto leading-relaxed px-2">
              Discover premium DTCP plots for your dream home
            </p>
